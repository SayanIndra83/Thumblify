...
import { ThumbnailStyle, thumbnailStyles } from './GenerateClient'
...
const styleIcons: Record<ThumbnailStyle, React.ReactNode> = {
  ...
  return (
    <div className='relative dark space-y-3'>
    <label className='block text-sm font-medium text-zinc-200'>Thumbnail Style</label>
      
      <button
      type='button'
      onClick={()=> setIsOpen(!isOpen)}
      className='flex w-full items-center justify-between rounded-md border px-4 py-3 text-left transition-all bg-white/8 border-white/10 text-zinc-200 hover:bg-white/12'
      >
        <div className='space-y-1 '>
            <div className='inline-flex gap-3 items-center'>{styleIcons[value]}
                <span>{value}</span>
            </div>
      ...
      {isOpen && (
        <div className='absolute bottom-0 z-50 mt-1 w-full rounded-md border border-white/12 bg-black/20 backdrop-blur-3xl shadow-lg'>
            {thumbnailStyles.map((style) => (
                <button key={style}
                type='button'
                onClick={(e) => {onChange(style); setIsOpen(false)}}
